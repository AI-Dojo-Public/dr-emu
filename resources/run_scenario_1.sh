...
# Create Worker
resultRaw=$(test "cryton-cli workers create attacker -f" "s/^.*'id': \(.*\),.*$/\1/p" "$checkWord" -1 "" 0)
...
# Validate template
resultRaw=$(test "cryton-cli plans validate resources/template.yml -i resources/inventory.yml" -1 "$checkWord" -1 "" 0)
IFS="ˇ" read -ra resultParsed <<< "$resultRaw"
echo "${resultParsed[2]}"


# Template create
resultRaw=$(test "cryton-cli plan-templates create resources/template.yml" "s/^.*'id': \(.*\),.*$/\1/p" "$checkWord" -1 "" 0)
IFS="ˇ" read -ra resultParsed <<< "$resultRaw"
...
# Plan create
resultRaw=$(test "cryton-cli plans create $templateId -i resources/inventory.yml" "s/^.*'id': \(.*\),.*$/\1/p" "$checkWord" -1 "" 0)
IFS="ˇ" read -ra resultParsed <<< "$resultRaw"
